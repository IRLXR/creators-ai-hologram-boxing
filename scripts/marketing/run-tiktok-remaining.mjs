#!/usr/bin/env node
/** Creates only remaining TikTok ads not yet in account. */
...
const PAYLOADS = JSON.parse(fs.readFileSync(path.join(ROOT, "scripts/marketing/tiktok-ad-payloads.json"), "utf8"));
...
function utmContent(creative, slug) {
  return { walk: `walk_inside_${slug}`, future_vo: `future_vo_${slug}`, future_mystery: `future_mystery_${slug}`, whatis: `whatis_${slug}` }[creative] || `${creative}_${slug}`;
}
...
async function callTikTok(tool_name, params) {
  ...
  const paths = { smart_plus_ad_create: "/smart_plus/ad/create/", smart_plus_adgroup_create: "/smart_plus/adgroup/create/" };
...
const summary = { campaigns: {}, adgroups_created: [], ads_created: [], errors: [] };
const adgroupIds = { "ADGRP_HB_Experience_FL_18-44": "1870330424155729" };
...
async function runOp(op) {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    if (result.code === 41021) { await sleep(2500 * (attempt + 1)); continue; }
    if (result.code !== 0) {
...
async function main() {
  ...
  for (const pack of ad_packs.founding_fan_4.slice(1)) {
    ops.push(buildAd({ adgroup_id: campaigns.founding_fan.adgroups.general, ad_name: `AD_HB_Convert_General_${pack.suffix}`, creativeKey: pack.creative, texts: pack.texts, utm_campaign: "founding_fan", utm_slug: "general_fl" }));
  }
  for (const pack of ad_packs.founding_fan_4) {
    ops.push(buildAd({ adgroup_id: campaigns.founding_fan.adgroups.family, ad_name: `AD_HB_Convert_FamilyNight_${pack.suffix}`, creativeKey: pack.creative, texts: pack.texts, utm_campaign: "founding_fan", utm_slug: "family_fl" }));
  }

  const expId = adgroupIds["ADGRP_HB_Experience_FL_18-44"];
  for (const ad of [
    ...
    ops.push(buildAd({ adgroup_id: expId, ad_name: ad.name, creativeKey: ad.creative, texts: pack.texts, utm_campaign: "experience", utm_slug: "experience" }));
  }
  ...
  for (const grp of awarenessGroups) {
    ...
    for (const pack of awarenessPack) {
      ...
      const built = buildAd({ adgroup_id: `__${grp.name}__`, ad_name: `AD_HB_Aware_${grp.segment}_${pack.suffix}`, creativeKey: pack.creative, texts: src.texts, utm_campaign: "awareness", utm_slug: grp.slug });
      ops.push(built);
    }
  }
